...
# River discharge, DOC and nitrate
...
The discharge (Q), dissolved organic carbon (DOC) and nitrate (NO~3~) data for Buck Creek are split across three datasets. We want to merge these datasets into one dataframe.

**1.** Download the three datasets and save to your working directory:

[Q.csv](https://raw.githubusercontent.com/gorsknm0/cbsi_r_training/refs/heads/main/data/Q.csv) 

[Sample_DOC.csv](https://raw.githubusercontent.com/gorsknm0/cbsi_r_training/refs/heads/main/data/Sample_DOC.csv) 

[Sample_Nitrate.csv](https://raw.githubusercontent.com/gorsknm0/cbsi_r_training/refs/heads/main/data/Sample_Nitrate.csv) 
...
From this we can see that all dataframes share the following columns: "site", "Date", "Julian", "Month", "Day", "DecYear", "MonthSeq"
...
colnames(df_comb) <- gsub(".y", "_Nitrate", colnames(df_comb))
```
...
[fake_dataset.csv](https://raw.githubusercontent.com/gorsknm0/cbsi_r_training/refs/heads/main/data/fake_dataset.csv) 
...
We can see from the diagnostic plots that our model is a very good fit...That is not a coincidence! CoPilot was specifically asked to to generate dataset with a significant **linear** relationship, with a high R^2^.But with real data it is unlikley that we would get such a good fit.
...
**7.** Let's now see what happens when we try to fit a linear model to the real world river data. Plot the mean DOC concentration (`df_comb$ConcAve_DOC`) against river discharge (`df_comb$Q`, measured in m^3^ s^-1^). So we are treating discharge as the predictor variable, and mean DOC concentration as the response variable.
...
plot(df_comb$Q, df_comb$ConcAve_DOC, col=cols[as.factor(df_comb$site)])
```

It looks like the relationship between DOC and discharge is different depending on the site.
...
```{r, echo=TRUE, eval=FALSE,collapse=TRUE}
LM_BC<-lm(df_comb$ConcAve_DOC[df_comb$site=="Buck Creek"]~df_comb$Q[df_comb$site=="Buck Creek"])
```
...
```{r, echo=TRUE, eval=FALSE,collapse=TRUE}
ANCOVA<-lm(df_comb$ConcAve_DOC~df_comb$Q*df_comb$site)
```
...
It is pretty much the same as the output of a normal linear model- you get an R^2^ value and a p-value. But in the coefficicents table, instead of one row for the intercept and one row for the slope, we have lots of rows. Initially it can be hard to interpret this part of the output, but it is actually quite straightforward once you know what you are looking at. Take a look at the below figure:

![ANCOVA Summary Output](figures/ANCOVA_SUMMARY_OUTPUT.png)
&nbsp; 
...
```{r, echo=TRUE, eval=FALSE,collapse=TRUE}
abline(5.5809,6.0679, col="blue")
```

And this for North Buck Creek:

```{r, echo=TRUE, eval=FALSE,collapse=TRUE}
abline((5.5809+11.9585),(6.0679-2.1527), col="darkblue")
```
...
abline((SUMMARY$coefficients[1,1]+SUMMARY$coefficients[3,1]),(SUMMARY$coefficients[2,1]+SUMMARY$coefficients[5,1]), col="darkblue")
```

Using this method, can you produce the abline code for South Buck Creek? 
...
```{r, echo=TRUE, eval=FALSE,collapse=TRUE}
ANCOVA_SIMPLIFIED<-lm(fake$Var2~fake$Var1+fake$Factor)
```
...
```{r, echo=TRUE, eval=FALSE,collapse=TRUE}
anova(ANCOVA, ANCOVA_SIMPLIFIED, test= F)
```
